...
Данная версия была написана на **php 8.4**
****
## Установка
...
Динамического url можно достичь, используя функционал переадресации.
...
***
## События
Данное ядро поддерживает систему событий.
Это означает что в ядре можно зарегистрировать событие, на которое могут подписываться неограниченное количество слушателей.
Затем, когда будет вызвано событие - будут вызваны все слушатели события.

P.S. Слушатели события находятся в рамках одного массива и при вызове перебираются последовательно. 
Крайне не рекомендуется делать большое количество слушателей в рамках одного события, так как это может привести к проблемам производительности (задержкам при вызове события).
...
Для создания собственного слушателя на событие, посмотрите реализацию слушателей встроенных в ядро (директория `EventListeners`).
...
Также модуль может иметь собственный (будет вызван после стандартного сценария установки) сценарий установки.
